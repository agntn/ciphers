...
IEEE dropped LRW from the final P1619 and went with [XTS](/ciphers/aes-xts). LRW breaks when the tweak key ends up encrypted under itself, which is easy to do on a disk. It also has no integrity check, so flipped bits decrypt to garbage without a word. Same as the ECB ciphers, this is plain TypeScript, not constant time, for puzzles and for seeing what a tweak does. Don't protect anything real with it.
