...
First argument not a subcommand, and not `--help` or `--version`? Then it's a cipher name and the CLI encodes. `ciphers caesar ...` and `ciphers encode caesar ...` are the same call, because typing `encode` every time gets old fast.
...
| `ciphers ciphers` | List the registered ciphers by category, `-v` adds the options, `-c classical` keeps one category |
| `ciphers info <cipher>` | One cipher's category, family, options and keyspace |
...
| --- | --- | --- |
| `--shift <n>` | `-s` | caesar |
| `--key <word>` | `-k` | vigenere, beaufort, autokey, alberti, playfair, polybius, columnar, adfgvx, bifid, aes, aes-cbc, aes-cfb, aes-ofb, aes-ctr, aes-ccm, aes-ocb, aes-lrw, aes-xts, aes-cbc-mac, rijndael, triple-des, triple-des-cbc and blowfish (hex) |
| `--transposition <word>` | | adfgvx |
| `--iv <hex>` | | aes-cbc, aes-cfb, aes-ofb, aes-ctr, triple-des-cbc |
| `--segment <bits>` | | aes-cfb |
| `--block-size <bits>` | | rijndael |
| `--nonce <hex>` | | aes-ccm, aes-ocb |
| `--aad <hex>` | | aes-ccm, aes-ocb |
| `--tag-length <bits>` | | aes-ccm, aes-ocb |
| `--tweak <hex>` | | aes-lrw, aes-xts |
