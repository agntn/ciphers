...
| Brute force | `create("caesar").decode(text, { shift })` for 1 to 25, best `analyzeFrequency(…, language).fit` first | `cipher_brute_caesar` |
...
The panels on the [home page](/) walk through the thirty-seven ciphers with one sample sentence each, computed the same way, in the browser, on one clock. Hover a panel to pause it. The arrows under the code step it by hand.
