...
| --- | --- | --- |
| `--shift <n>` | `-s` | caesar |
| `--key <word>` | `-k` | vigenere, beaufort, autokey, alberti, playfair, polybius, columnar, adfgvx, bifid, aes, aes-cbc, aes-cfb, aes-ofb, aes-ctr, aes-ccm, aes-lrw and triple-des (hex) |
| `--transposition <word>` | | adfgvx |
| `--iv <hex>` | | aes-cbc, aes-cfb, aes-ofb, aes-ctr |
| `--segment <bits>` | | aes-cfb |
| `--nonce <hex>` | | aes-ccm |
| `--aad <hex>` | | aes-ccm |
| `--tag-length <bits>` | | aes-ccm |
| `--tweak <hex>` | | aes-lrw |
