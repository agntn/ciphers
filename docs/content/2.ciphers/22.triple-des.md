...
That is [DES](/ciphers/des) with `133457799bbcdff1` and nothing more, 56 bits of key. `openssl enc -des-ecb -provider legacy -K 133457799bbcdff1` prints the same bytes. It's the whole point of the EDE order: one Triple DES implementation still speaks plain DES.
