...
The command prints the shifts in order. `cipher_brute_caesar` returns the same 25 sorted by `fit`, the most English line first, or the most Polish one with `lang: "pl"`. A model then reads the answer on line one instead of wading to shift 17. It counts letters, it doesn't know words, so a two-word text can lose by a line: `KHOOR ZRUOG` puts `EBIIL TLOIA` above `HELLO WORLD`. That's why all 25 still come back.
