...
- `des` is not a Triple DES mode either, so it has its own `src/ciphers/block/des.ts` and runs `desBlock` from `triple-des/block.ts`, the same rounds Triple DES runs three times.
